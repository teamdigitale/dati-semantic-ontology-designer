...
export default function edgeEditing(grapholscape: Grapholscape) {
  ...
  if (!cy || cy.scratch('edge-editing-enabled'))
    return
  ...
  const onEditingCompleted = () => {
    if (selectedEdge && grapholscape.renderState) {
      removeAnchorsOnEdge(selectedEdge)
      selectedEdge.removeClass('editing')

      const grapholEdge = grapholscape
        .renderer
        .diagram
        ?.representations
      ...
      if (grapholEdge?.isEdge()) {
        grapholEdge.sourceId = selectedEdge.source().id()
        grapholEdge.targetId = selectedEdge.target().id()
      }
    }
  ...
  cy.scratch('edge-editing-enabled', true)
}