import { GrapholEntity, Diagram, Ontology, Annotation, Lifecycle, IonEvent, IEventTriggers } from "grapholscape"
import type { RDFGraph } from "../gen"
...
export interface IonDesignerEvent extends IonEvent {
  ...
  (event: DesignerEvent.AssistantRequest, callback: () => void): void
}
...
export interface IDesignerTriggers extends IEventTriggers {
  ...
  (event: DesignerEvent.AssistantRequest): void
}
...
export class DesignerLifeCycle extends Lifecycle {
  ...
  private assistantRequest: (() => void)[] = []

  declare trigger: IDesignerTriggers
